  ...
  >
    <template #default>
      <div class="row q-py-xs">
        ...
        <template v-else>
          <div class="col-auto text-3 text-weight-medium">
            {{ mapLabel(model) }}
          </div>
        </template>
      ...
      >
        ...
        <EzScrollbar v-if="_options.length" height="175px">
          <div
            v-for="option in _options"
            :key="option.id"
            class="q-pa-md hoverable text-3"
            :class="
        ...
        <div v-else class="q-pa-md text-gray-5 text-3 text-center">
          {{ addNewOption ? 'Press "Enter" to add' : "No data found" }}
        </div>
...
const EzInput = () => import("@/library/components/ez-input/index.vue");
const EzScrollbar = () => import("@/library/components/ez-scrollbar/index");

export default {
  name: "EzSelect",

  components: { EzField, EzInput, EzScrollbar },
